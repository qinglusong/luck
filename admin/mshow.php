...
$smarty->assign('cur', 'mshow');
$smarty->assign('ur_here', $_LANG['mshow']);
$systems = 'mobile';
...
elseif ($rec == 'insert') {
    ...
    $dou->dou_msg($_LANG['show_add_succes'], 'mshow.php');
} 
...
elseif ($rec == 'update') {
    ...
    $dou->dou_msg($_LANG['show_edit_succes'], 'mshow.php');
}
...
elseif ($rec == 'del') {
    // 验证并获取合法的ID
    $id = $check->is_number($_REQUEST['id']) ? $_REQUEST['id'] : $dou->dou_msg($_LANG['illegal'], 'mshow.php');
    ...
    if (isset($_POST['confirm']) ? $_POST['confirm'] : '') {
        ...
        $dou->delete($dou->table('show'), "id = '$id'", 'mshow.php');
    } else {
        $_LANG['del_check'] = preg_replace('/d%/Ums', $show_name, $_LANG['del_check']);
        $dou->dou_msg($_LANG['del_check'], 'mshow.php', '', '30', "mshow.php?rec=del&id=$id");
    }
}
